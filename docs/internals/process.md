...
The important structural fact: **every yield goes through a graph `source`**.
The local update path is therefore identical to the wire's — `reconcile` runs
on the yield either way — which is why a remote process behaves exactly like a
...
`error` is a plain closure variable, not part of the published meta — the
...
drop-newest — under sustained overload the freshest input survives.
...
`close()` resolves parked takers as done and drops the queue — which ends the
...
and clears the meta flags. `r.done` ends the loop — **a process that returns is
...
instance; pending asks do not — they already rejected.
...
before awaiting.** `unscoped()` is the deliberate escape hatch — the registry
...
async disposal — the honest consequence of cooperative cancellation.
...
or binding and a plain snapshot pull anywhere else — the process handle
inherits path precision for free from [graph.ts](graph.md).

`ask(msg)` builds `{ ...msg, reply }`, registers the rejector under that exact
object, and pushes it. The generator sees an ordinary message carrying `reply`;
...
Lossy is the right default for state sync — it is also exactly what the wire
...
retained after dispose — needs `gc({ execution: 'async' })`, since plain `gc()`
false-fails under V8 conservative stack scanning), `types.check.ts` (the
`@ts-expect-error` lines are load-bearing regression checks on the type
surface).

Next: [registry.md](registry.md) — naming, sharing, and eviction.
