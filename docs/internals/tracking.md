...
Proxies handed out during the run may outlive it — application code can store
one. After `finalize()` the `get` trap short-circuits to `Reflect.get`, so a
stale proxy is an ordinary read-only view: correct values, no phantom
...
| `traversed` | a container was obtained and read into | nothing by itself — only the reads beneath it |
| `subtree` | a container escaped the reader | any op at or below this path |

The distinction that does the real work is **traversed vs subtree**. A reader
that walks into `state.items[3].done` and reads a boolean depends on that
boolean — not on `items`, and not on `items[3]`. A reader that grabs
...
is not a trappable operation. So escape is inferred at `finalize()` — a node
...
- **Frozen data properties are a coarse fallback.** A proxy must return the
  ...
  instead — less precise, still correct.
...
Two cases at the end of a path are worth spelling out:
...
gate — O(watchers). An inverted path index would make it O(affected), and the
...
Tests: `packages/core/test/graph.test.ts` — "path intersection boundaries
...
Next: [graph.md](graph.md) — how these trees become subscriptions.
