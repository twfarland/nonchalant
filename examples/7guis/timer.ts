...
type TimerMsg = { type: 'tick' } | { type: 'duration'; s: number } | { type: 'reset' }

const timer: Proc<TimerState, TimerMsg, void> = async function* (self) {
  let duration = 10
  let elapsed = 0
  const iv = setInterval(() => self.send({ type: 'tick' }), 100)
...
mount(document.getElementById('app')!, div({ class: 'card' },
  ...
  div({},
    ...
    input({
      ...
      oninput: (e: Event) => t.send({ type: 'duration', s: Number((e.target as HTMLInputElement).value) }),
    })),
  button({ onclick: () => t.send({ type: 'reset' }) }, 'Reset')))
