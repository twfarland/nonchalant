...
describe('the shop schema', () => {
  it('rename is an ask: write-through to the user, one ripple to the list', async () => {
    const users = shop.lookup('users')
    const ada = shop.lookup('user', { id: 1 })
    await until(() => users() !== undefined && ada() !== undefined)
    expect(ada().name).toBe('Ada')

    const updated = await ada.ask({ type: 'rename', name: 'Ada Lovelace' })
  ...
  it('a failed write rejects the ask; the restart policy re-syncs from the server', async () => {
    const ada = shop.lookup('user', { id: 1 })
    await until(() => ada() !== undefined)
    const before = ada().name

    await expect(ada.ask({ type: 'rename', name: '' })).rejects.toThrow(/name is required/)
