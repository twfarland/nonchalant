...
describe('call / reply', () => {
  it('round-trips a typed call', async () => {
    ...
    p.cast({ type: 'add', n: 5 })
    const res = await p.call({ type: 'sum' })
    expect(res).toEqual({ sum: 15 })
    p[Symbol.dispose]()
  })

  it('rejects pending calls on crash; readers keep the last value, stale', async () => {
    const p = spawn(cart, { start: 1 }, { initial: 1 })
    p.cast({ type: 'add', n: 1 })
    await tick()
    expect(p()).toBe(2)
    p.cast({ type: 'boom' })
    const q = p.call({ type: 'sum' }) // queued behind the crash
    await expect(q).rejects.toThrow('boom')
    await tick()
    expect(p()).toBe(2) // last value retained
    expect(p.stale).toBe(true)
    expect(p.error).toBeInstanceOf(Error)
    await expect(p.call({ type: 'sum' })).rejects.toThrow(/crashed/)
    p[Symbol.dispose]()
  })

  it('rejects calls after normal completion', async () => {
    const p = spawn(
    ...
    await tick()
    await expect(p.call({ type: 'q' })).rejects.toThrow(/done/)
    p[Symbol.dispose]()
  })
})
...
describe('restart policies', () => {
  it('on-crash restarts from init args and replays queued casts', async () => {
    ...
    expect(p()).toBe(101)
    p.cast({ type: 'boom' })
    p.cast({ type: 'add', n: 7 }) // queued during the crash; replays into the new instance
    await tick()
    await tick()
...
describe('mailbox modes', () => {
  ...
  it('a bounded mailbox drops oldest on overflow', async () => {
    ...
    await tick()
    for (const n of [1, 2, 3, 4, 5]) p.cast(n)
    await new Promise((resolve) => setTimeout(resolve, 30))
    expect(seen).toEqual([4, 5])
    p[Symbol.dispose]()
  })

  it('self.cast posts to the own inbox', async () => {
    const p = spawn(
      async function* (self: Self<'kick' | 'finish'>) {
        yield 'idle'
        for await (const m of self) {
          if (m === 'kick') self.cast('finish')
          else yield 'finished'
        }
      },
      undefined,
